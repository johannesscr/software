...
func cloneGraph(node *Node) *Node {
	...
	clone := dfs(node, &Node{Val: node.Val}, cloned)
	return clone
}

func dfs(node, clone *Node, cloned map[int]*Node) *Node {
	cloned[clone.Val] = clone
	for _, nei := range node.Neighbors {
		if cloneNei, ok := cloned[nei.Val]; ok {
			// node already visited
			clone.Neighbors = append(clone.Neighbors, cloneNei)
		} else {
			clone.Neighbors = append(clone.Neighbors, dfs(nei, &Node{Val: nei.Val}, cloned))
		}
	}
	return clone
}
